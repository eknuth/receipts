...
1. **Orient.** Call `get_workspace_context` first. Then look at what the dataset actually holds:
   ...
   back with nothing. Call `get_triggers` once as well, to learn which alerts exist and whether
   any is firing right now. A firing trigger is a lead worth testing with queries, not proof of
   the cause: its state is a fact about the present, not about why.
