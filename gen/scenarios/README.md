...
| `control-noisy` | control | none; every span's spread is doubled | one minute burst of paypal `payments.charge` errors at minute 4 that resolves on its own | no incident; the noise and the burst are both explained, neither is ongoing |
...
| `herring-customer-whale` | red herring stronger by count | `payments.charge` fails 20% on adyen, from minute 10 | cust-00007 is 15% of traffic and about 30% of all errors over the whole window, with its own failure rate about 11x the rest before onset | adyen is the incident: excluding the whale, adyen's error rate still steps at onset; excluding adyen, the whale's own rate holds flat across onset |
