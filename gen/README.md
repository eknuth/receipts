...
| `ground_truth.equivalent_dims` | no | Alternative selectors that name the same population as `root_cause_dims`, declared deliberately per scenario, not inferred. Checked against the topology at load time: a `name` clause must equal `fault.effect.span`, a `service.component` clause must equal the service that runs it, and any other key must repeat a `fault.where` dimension with the same value; an alternative that repeats `root_cause_dims` exactly is rejected as redundant, and a control may not declare it. The grader scores the top hypothesis's dims against `root_cause_dims` and against every entry here and keeps the best. |
