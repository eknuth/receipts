...
| `ground_truth.symptom_dims` | never, it is derived | Not a field. `Scenario.symptom_dims` reads the fault and returns the pairs the generator wrote somewhere in the trace of a request the fault slowed or failed: the effect's span as `name`, the service that runs it, and, when the effect fails the span, `error`, `http.status_code`, and whatever `error.type` or `exception.type` it attached. A file that writes `symptom_dims` is rejected. The grader treats a reported pair that is a symptom and is not in `root_cause_dims` as neither right nor wrong; see `evals/grader.md`. |
